...
- 함수
  - `bool TryPlaceOrder(EItemType ItemType, int32 Quantity)` (8단계 — 전용 플레이어 개념이 없어져 `AFactoryPlayerController::Server_SubmitKioskOrder` 경유로 누구나 호출 가능. Docs 이탈, 승인됨 — 재고 잠금 체크 통과 후 쿨다운(`AMSmartFactoryManager::ReorderCooldownSeconds`) 미경과 시 실패, 통과 시 `AMSmartFactoryManager::GetUnitPrice(ItemType) * Quantity`만큼 `TryAdjustFunds`로 공용 자금 차감(잔액 부족 시 주문 자체를 진행하지 않고 실패). 성공 시 쿨다운 타임스탬프 갱신 + `PendingInboundQuantities[ItemType] += Quantity` 후 `TryDrainInboundBacklog` 1회 시도)

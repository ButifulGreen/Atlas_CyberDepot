...
- 함수
  ...
  - `void AssignHomeIdleZoneSlots()` (7단계 후속, private — "대기실은 선입선출이 아니라 실행 시 1회 고정 배정" 규칙. 버그 수정(사용자 지시) — 원래는 `EActorType`(Atlas/TransportRobot)별로 로봇/대기실 풀을 완전히 분리해 각각 이름순으로 정렬한 뒤, 대기실을 순서대로 순회하며 `AIdleWaitingZone::AssignHomeSlots`로 자기 마커 개수만큼 로봇을 앞에서부터 소비시켰다(물리적 위치와 무관). `AIdleWaitingZone::AllowedAgentTypes`가 단일값에서 비트마스크로 바뀌어 한 대기실이 아틀라스·배송로봇을 동시에 받을 수 있게 되면서 타입별 분리 풀 자체가 부적절해졌고, 이 참에 배정 기준도 바꿨다 — 이제 레벨의 모든 로봇 + 모든 대기실의 모든 슬롯(`AIdleWaitingZone::GetParkingSlotLocations`로 조회)을 한 번에 모아, "아직 안 정해졌고 `AIdleWaitingZone::IsUsableBy`로 타입이 허용되는" 로봇-슬롯 쌍 중 **`FVector::DistSquared`로 가장 가까운 조합**을 하나씩 그리디하게 확정한다(동률 시 이름순 정렬로 실행마다 동일한 결과 보장). 대기실 마커 총합이 항상 로봇 수 이상이라고 가정 — 로봇이 남는 경우는 경고 로그만 남기고 넘어간다)

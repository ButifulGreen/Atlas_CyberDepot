...
- (7단계 후속 신규) 자연 발생 고장(`EvaluateRotationOrContinue`의 확률 롤, 항상 `Working` 도중 발생하므로 `CurrentAssignment`/`CurrentTask`가 유효한 상태)에서 복구된 로봇은 `SetState(Idle)`만 될 뿐, 보존된 배정/트립을 실제로 이어서 재개하는 로직이 없다 — "예약을 유지한 채 수리 후 재개"가 설계 원칙(`00_DesignPrinciples.md`, 이 문서 하단 미채택 사유 참고)인데도 그 재개를 실제로 트리거하는 코드가 빠져있다. `ResumeAfterRepair()` override는 배정이 없던(디버그 강제 고장 등) 경우의 유휴 재배차만 처리하도록 일부러 좁혀뒀다 — 배정이 남아있는 채로 넘기면 `TryDispatchIdleAgents()`가 새 배정을 덮어씌워 원래 슬롯 예약이 고아가 될 위험이 있어서다. 실제 레벨에서 자연 고장이 잦아지는 단계(밸런싱)에서 재검토 필요.

...
- 멤버: `FGuid AgentID`, `FString DisplayName`(사용자 지시, 신규 — 아웃라이너 표시 이름(`GetActorLabel`)과 로그 이름(`GetName`, 런타임 스폰 시 자동 생성되는 내부 UObject 이름)이 서로 달라 특정 로봇을 찾기 어려운 문제 해결용. `ReplicatedUsing = OnRep_DisplayName`. 서버가 `BeginPlay`에서 `AMSmartFactoryManager::AllocateNextAgentDisplayNumber`로 타입별 일련번호를 받아 `"Atlas-03"` 형태로 한 번만 채우고, 이후 이 클래스 전체의 로그가 `GetName()` 대신 이 값을 쓴다. `OnRep_DisplayName`(서버는 명시 호출, 클라이언트는 리플리케이션 자동 호출)이 `SetActorLabel`도 같이 호출해(`#if WITH_EDITOR`) 아웃라이너 표시도 통일한다), `EAgentType AgentType`, `EAgentState CurrentState`, `FVector TargetLocation`, `float BlockedTimer`, `static constexpr float BlockedThresholdSeconds = 2.f`, `bool bIsParkedInIdleZone`(`AIdleWaitingZone::TryOccupyHomeSlot`/`ReleaseSlot`이 갱신, "지금 앉아있음"), `TWeakObjectPtr<AIdleWaitingZone> HomeIdleZone` + `int32 HomeSlotIndex`(7단계 후속 — 선입선출 없이 레벨 시작 시 1회만 고정 배정되는 홈 대기실/슬롯. `UOutboundDispatchSubsystem::AssignHomeIdleZoneSlots`가 채운 뒤로는 세션 내내 유지됨), `bool bIsHeadingToIdleZone`(7단계 후속 — 대기실로 이동 중인 동안만 true, 도착하면 즉시 false. `bIsParkedInIdleZone`과 달리 "이동 중"만을 뜻해 `OnArrivedAtDestination`이 대기실행 도착 여부를 판별하는 데 쓰인다. 위 세 값 모두 `CurrentAssignment`와 동일하게 리플리케이트하지 않음)
- `DrawDebugOperationCountLabel()`(private, `GetRepairComponent()`가 있는 Atlas/TransportRobot 전용)이 `DisplayName`+`GetOperationCount()`를 캡슐 위에 같이 표시한다. 갱신 주기는 버그 수정(사용자 리포트) — 기존 1초는 로봇이 이동하면 표시가 허공에 남았다가 새 위치에 다시 그려지는 것처럼 보여, 0.1초로 단축(`DrawDebugString` `Duration`도 0.15초로 맞춰 다음 갱신 전까지 끊기지 않게 함). 버그 수정(사용자 리포트, 2차) — `DrawDebugString`은 `SetActorHiddenInGame`과 무관하게 계속 그려져서(월드 좌표에 직접 찍는 별개 호출이라 컴포넌트 가시성을 안 따름), 리플레이 재생 중 실제 로봇을 숨겨도(`Docs/10_Benchmark_Replay.md` 참고) 이 이름표만 허공에 남아있었다. `IsHidden()`이면 아예 그리지 않도록 조기 반환 추가 — 대신 그 시간 동안 고스트 액터(`AReplayGhostActor::NameLabel`)가 같은 이름표를 대신 띄운다.
- 함수

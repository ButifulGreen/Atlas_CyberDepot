...
- 함수
  ...
  - `void RequestMoveWithFilter(const FVector& Destination)` (이동 요청 직전 `ApplyDynamicCongestionCost`를 통해 `QueryFilterClass` 인스턴스의 AreaCost를 현재 혼잡도 기준으로 갱신한 뒤 이동 요청. 목적지가 바뀌면 재시도 카운트를 리셋, 같은 목적지 재시도(`RetryLastMove`)면 유지)
...
- 함수
  ...
  - `void OnArrivedAtDestination()` (override — 맨 앞에서 `TryHandleWaypointRouteArrival()`(Docs 이탈, 승인됨 — `08_Navigation.md` §8-B)을 가장 먼저 확인해 웨이포인트 경로 중간/최종 홉 이동 중이면 즉시 반환, 그 다음 `TryHandleIdleZoneArrival()`(7단계 신규)을 확인해 대기실 도착이면 즉시 반환, 아니면 `PendingHandoffAssignmentID` 확인 후 기존 배정 로직 진행)

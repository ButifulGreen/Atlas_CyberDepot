...
- 함수
  ...
  - `void OnRepairCompleted()` (`OperationCount` 리셋, Broken/bMaintenanceDue 상태 해제, `FAnomalyEvent` 발행, `AMSmartFactoryManager::OnRepairCompleted` 호출. 버그 수정 — 정비자 목록을 통보 없이 비우면 AI NPC가 `UnderRepair`에 영구히 멈춰 `FindNearestAvailableNPC` 후보에서 계속 빠지는 문제가 있어, 목록을 비우기 전 AI 제어 중인 NPC 정비자에게 `AFactoryNPCHuman::ReturnToOfficeRoom()`을 자동 호출하도록 함. 단, `AMSmartFactoryManager::TryAssignNextPendingMaintenance`로 대기열에 다른 정비 필요 로봇이 있는지 먼저 확인해 있으면 그쪽에 재배정하고, 없을 때만 사무실로 복귀시킨다. 빙의 중인 플레이어는 제외 — 8단계에서 정교화)
